...
function twoStage(base, { r, g1, gT, years }) {
...
export function runModels(inputs, assumptions) {
  const out = {};
...
export const clamp = (x, lo, hi) => (x == null ? null : Math.min(hi, Math.max(lo, x)));
...
export function multiplesAt(price, ps, netDebtPs) {
...
export function impliedPrice(def, multiple, ps, netDebtPs) {
