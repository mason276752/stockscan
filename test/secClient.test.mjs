...
import { SecClient } from '../server/lib/secClient.js';
...
test('sec.gov requests are at least 101 ms apart, however they are issued', async () => {
  ...
  for (const gap of parallel) assert.ok(gap >= 101, `gap ${gap.toFixed(1)} ms < 101 ms`);
  ...
  for (const gap of mixed) assert.ok(gap >= 101, `gap ${gap.toFixed(1)} ms < 101 ms`);
});
...
test('a retry waits its turn too', async () => {
  ...
  assert.ok(at[1] - at[0] >= 101, `retry came ${(at[1] - at[0]).toFixed(1)} ms after the first attempt`);
});
