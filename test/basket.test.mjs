...
import { basketRequest, basketSeries } from '../server/lib/basket.js';
...
const member = (symbol, price, days = DAYS) => ({ symbol, weight: 1, source: 'test', days: days.map((date) => ({ date, open: price(date), high: price(date), low: price(date), close: price(date), volume: 100 })) });
