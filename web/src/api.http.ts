...
import type { Api, Params, StreamEvent } from './apiTypes.ts';

// a params object as a query string: what is not set does not go on the URL
const qs = (params: Params) => new URLSearchParams(Object.entries(params).filter(([, v]) => v !== undefined).map(([k, v]) => [k, String(v)]));
...
export const api: Api = {
  ...
  indicators: (id, params) => get(`/api/company/${encodeURIComponent(id)}/indicators?${qs(params)}`),
  indicatorsUrl: (id, params) => url(`/api/company/${encodeURIComponent(id)}/indicators?${qs(params)}`),
  valuation: (id, params) => get(`/api/company/${encodeURIComponent(id)}/valuation?${qs(params)}`),
  valuationUrl: (id, params) => url(`/api/company/${encodeURIComponent(id)}/valuation?${qs(params)}`),
  warmup: () => [], // the server holds the indexes; nothing to warm
  status: () => get('/api/status'),
  screenFields: () => get('/api/screen/fields'),
  screen: (params) => get(`/api/screen?${qs(params)}`),
  screenUrl: (params) => url(`/api/screen?${qs(params)}`),
  ...
  browseCompanies: (params) => get(`/api/browse/companies?${qs(params)}`),
...
async function stream(path: string, body: unknown, onEvent: (e: StreamEvent) => void, signal?: AbortSignal): Promise<void> {
