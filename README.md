...
| `POST /api/basket/rule` `{params:{<尋找股票的查詢>}, benchmark, minPrice}` | 規則型 ETF：條件在每個財報日重播出來的指數日 K、統計、每一次加入 / 剔除的日期、歷來成分股與各自的持有期間 |
...
- **一檔都不符合的期間**：指數空手持平（不是斷線），並註明有幾個交易日是這樣。
  ...
  不算下市，收盤價往後補到最後一天。
- **頁面**：上方是同一張 K 線圖與統計（區間固定是全期間、權重固定等權重，所以沒有區間與再平衡選項，也不能用 TradingView 價差 widget——
...
- 上限 900 檔（歷來持有過的檔數）：再多光是日線就要抓幾十 MB。超過會直接說，請把條件收緊。
