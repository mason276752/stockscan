...
| `POST /api/basket/rule` `{params:{<尋找股票的查詢>}, range｜from+to, benchmark, minPrice, rebalance, weighting, maxWeight}` | 規則型 ETF：條件重播出來的指數日 K、統計、每一次加入 / 剔除的日期、歷來成分股與各自的持有期間。`rebalance` = `monthly`（預設）/ `quarterly` / `yearly` / `filing`；`weighting` = `equal`（預設）/ `cap`，`maxWeight` 是市值加權時單一個股的上限（0–1，預設 0.1） |
...
能設定的是**多久換一次股**與**權重怎麼分**（等權重，或市值加權並對單一個股設上限）。
...
- **權重**：預設**等權重**。另一種是**市值加權**——用換股當天的市值，也就是「當天最新那份財報的稀釋股數 × 當天股價」，
  兩個數字在當下都已經公開，不是事後諸葛——並對單一個股設**上限**（預設 10%），超出的部分按比例分給其他檔、
  反覆到全部都在上限內；上限填 100% 就是純市值加權。實測一組條件三年：等權重 +31%、市值加權上限 10% +43%、
  純市值加權 +206%（整段被一檔大贏家帶起來，這也是為什麼預設要設上限）。
