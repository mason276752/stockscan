...
- **只收在正式交易所掛牌的股票**（Nasdaq、NYSE、NYSE American、CBOE…），**OTC（櫃買/粉單）一律不收**：OTC 的報價常常不是真的能成交的價格——一分錢以下的最小跳動就是 100%，殼公司的報價根本沒有成交——照它畫出來的圖是報價雜訊不是報酬率。
  實測存檔的日線：OTC 每 4 檔就有 1 檔出現過「單日 4 倍、隔天原路跳回」的假跳動，NYSE 是 200 檔才 1 檔。交易所欄位是空白的（剛上市、SPAC，EDGAR 還沒填）就看該公司申報清單裡的交易所，也沒有才留著。
...
- 財報之外：財務指標與評分、股價估值、分類瀏覽（SIC / 申報身分 / ETF 成分股）、尋找股票、觀察名單，
  ...
  或把篩選條件本身做成**規則型 ETF**——從最早的財報一天一天重播，當天符合條件的等權重持有，列出每一次進出。
...
| `POST /api/basket/rule` `{params:{<尋找股票的查詢>}, range｜from+to, benchmark, minPrice}` | 規則型 ETF：條件在每個財報日重播出來的指數日 K、統計、每一次加入 / 剔除的日期、歷來成分股與各自的持有期間 |
...
從最早的一份財報開始一天一天重播——某天有公司發財報而符合條件就**買進**，發的財報讓它不符合就**剔除**，
當下持有的全部**等權重**。所以持股、權重、每一次進出的日期都是條件的函數，頁面上**不能手動增減成分股或調權重**
（能改的只有條件：來源列的「✎ 編輯篩選條件」回到尋找股票，改完按「更新規則」整段歷史重算）。
...
- **什麼時候成交**：財報在 EDGAR 公開的**下一個交易日開盤**（EDGAR 收件收到紐約時間晚上 10 點，
  當天收盤買等於先看到明天的報紙）。每次異動就把當下全部持股重新等分：賣出的按當天開盤價出場（已經沒有報價的按最後收盤價），
  湊起來的錢平均買進新名單。所以指數的開盤價 = 前一刻的市值，K 線是連續的。
...
- **最低股價（預設 1 美元）**：財報條件篩不掉報價 0.000001 美元的 OTC 空殼股，而那種股票**跳一檔就是 100%**
  ...
  擋掉之後是 258%）。所以低於這個價錢的成分股「符合條件但不買」，頁面上列出是哪幾檔、各被擋了幾天；
...
- **看某一段**：預設是全期間（從第一份符合條件的財報算起），也可以選 1 / 3 / 5 / 10 年或自訂起訖日，跟自製 ETF 同一組按鈕。
  ...
  這也是把 900 檔上限撐開的辦法：整段歷史太寬的條件，縮到幾年內往往就進得去。
- **頁面**：上方是同一張 K 線圖與統計（權重固定等權重，所以沒有再平衡選項，也不能用 TradingView 價差 widget——
...
- 上限 900 檔（區間內持有過的檔數）：再多光是日線就要抓幾十 MB。超過會直接說，請把條件收緊或把區間縮短。
