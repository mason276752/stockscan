...
//
// The custom ETF of basket.ts is a list of stocks the user keeps by hand.
...
//     the same as-of index (screen.ts): the server reads it from the store,
...
// basket.ts already calls illiquid - and the ones this kept out are named
...
const daysBefore = (date: IsoDate, n: number): IsoDate => new Date(Date.parse(date) - n * 86400000).toISOString().slice(0, 10);
...
export function replaySchedule(core: readonly ScreenRow[] | ScreenColumns | ScreenTable, index: AsOfIndex, q: ScreenQueryParams): RuleSchedule {
  ...
  const view: ScreenTable & { i: number; cur: AsOfRef | null; prev: AsOfRef | null; yoy: AsOfRef | null } = {
    length: 1,
    i: 0,
    cur: null,
    prev: null,
    yoy: null,
    row: () => {
      throw new Error('the replay view never materialises a row');
    },
    cik: () => t.cik(view.i),
  ...
  const changes: { date: IsoDate; ticker: string; add: boolean }[] = [];
  let tested = 0;
  for (let i = 0; i < t.length; i++) {
    ...
    // does with it.
    const dates = [...new Set(list.filter((f) => f.filingDate).map((f) => f.filingDate))].sort() as IsoDate[];
    if (!dates.length) continue;
    view.i = i;
    let held = false;
    const spans: { from: IsoDate; to: IsoDate | null }[] = [];
    const shares: { date: IsoDate; value: number }[] = [];
...
export function windowSchedule(schedule: RuleSchedule, { from = null, to = null }: { from?: IsoDate | null; to?: IsoDate | null } = {}): RuleSchedule {
  if (!from && !to) return schedule;
  const open = new Set<string>(); // held by the rules when the window opens
  const events: RuleEvent[] = [];
...
export const NDX_CONSTRAINTS: WeightConstraints = {
...
export const singleCap = (maxWeight: number): WeightConstraints => ({ single: { over: maxWeight, to: maxWeight, watch: Math.min(1, maxWeight * 1.2) } });

const total = (xs: readonly number[]) => xs.reduce((a, b) => a + b, 0);
// `v` scaled to add up to `t`, with no entry above `ceiling`
function spread(v: readonly number[], t: number, ceiling: number): number[] {
  const s = total(v);
...
function groupTo(w: readonly number[], inGroup: readonly boolean[], target: number, ceiling = Infinity): number[] {
  const gi: number[] = [];
  const oi: number[] = [];
...
const topIndexes = (w: readonly number[], n: number) =>
  w
    .map((x, i) => i)
    .sort((a, b) => w[b]! - w[a]!)
    .slice(0, n);
...
export function liveStages(w: readonly number[], c: WeightConstraints): Record<'single' | 'cohort' | 'top', boolean> {
  const n = w.length;
...
export function constrainWeights(w: readonly number[], c: WeightConstraints | null | undefined): number[] {
...
export function breachesConstraints(w: readonly number[], c: WeightConstraints | null | undefined): boolean {
...
export function capWeights(w: readonly number[], cap: number): number[] {
...
export function marketWeights(values: readonly number[], constraints: WeightConstraints | null | undefined): number[] {
...
export function rebalanceSessions(dates: readonly IsoDate[], every: string): Uint8Array | null {
  if (every === 'filing') return null;
  const bucket = (d: IsoDate) => (every === 'yearly' ? d.slice(0, 4) : every === 'quarterly' ? `${d.slice(0, 4)}Q${Math.floor((Number(d.slice(5, 7)) - 1) / 3)}` : d.slice(0, 7));
...
/** How the index is traded: the window, the schedule, and the weighting. */
export interface RuleSeriesOptions {
  base?: number;
  minPrice?: number;
  from?: IsoDate | null;
  to?: IsoDate | null;
  ...
  weighting?: string;
  maxWeight?: number;
  constraints?: WeightConstraints | null;
  special?: boolean;
}

export function ruleSeries(members: readonly RuleBarsMember[], events: readonly RuleEvent[], { base = 100, minPrice = MIN_PRICE, from = null, to = null, rebalance = 'monthly', weighting = 'equal', maxWeight = MAX_WEIGHT, constraints = null, special = true }: RuleSeriesOptions = {}) {
  ...
  const px = (i: number, k: number, f: 'open' | 'high' | 'low' | 'close') => {
  ...
  const counts: { date: IsoDate; n: number; special?: true }[] = []; // { date, n } whenever the number of holdings changes
  const per = rows.map(() => ({ days: 0, ret: 1, spells: 0, first: null as IsoDate | null, last: null as IsoDate | null, prevClose: 0, in: false, cheap: 0, wild: null as { date: IsoDate; factor: number } | null }));
  ...
  const sharesAt = (i: number, k: number) => {
  ...
  const skipped = new Set<string>(); // constraint stages this index is too small to meet
  ...
  const sell = (i: number, price: number) => {
    if (!per[i]!.in) return;
    ...
    if (rebalancing) {
      ...
      if (now!.length) {
        ...
        if (limits) {
          ...
          for (const stage of ['cohort', 'top'] as const) if (limits[stage] && !live[stage]) skipped.add(stage);
      ...
      if (forced) specials++;
      forced = false;
      counts.push({ date: d, n: now!.length, ...(on && !on[k] ? { special: true as const } : {}) });
  ...
  const wild = rows.map((m, i) => (per[i]!.wild ? { symbol: m.symbol, ...per[i]!.wild! } : null)).filter((x): x is { symbol: string; date: IsoDate; factor: number } => !!x);
  ...
  const constituents: RuleConstituent[] = rows.map((m, i) => ({
    symbol: m.symbol,
    source: m.source,
    days: per[i]!.days,
    spells: per[i]!.spells,
...
export function ruleRequest(body: Record<string, unknown>, today: IsoDate = new Date().toISOString().slice(0, 10)): RuleRequest {
  const params = body?.params && typeof body.params === 'object' ? (body.params as ScreenQueryParams) : null;
  ...
  if (!from && RANGES[String(body?.range)]) from = yearsBefore(to || today, RANGES[String(body.range)]!);
  return {
    params,
    from,
    to,
    range: from || to ? String(body?.range || 'custom') : 'all',
    ...
    rebalance: REBALANCE.includes(String(body?.rebalance)) ? String(body.rebalance) : 'monthly',
    weighting: WEIGHTING.includes(String(body?.weighting)) ? String(body.weighting) : 'equal',
...
export async function runRuleEtf(schedule: RuleSchedule, req: RuleRequest, fetchBars: (symbol: string) => Promise<{ source?: string | null; currency?: string | null; days: Bar[] }>, { emit = null, signal = null, extra = {} }: RunRuleOptions = {}) {
  ...
  const out = new Array<RuleBarsMember | undefined>(jobs.length);
  let next = 0;
  let done = 0;
  ...
  const worker = async () => {
    while (next < jobs.length && !signal?.aborted) {
      ...
      try {
        ...
        out[i] = { symbol, error: (err as Error).message, days: [] };
      }
      done++;
      const m = out[i]!;
  ...
  const shares = new Map<string, { date: IsoDate; value: number }[]>();
  ...
  const series = ruleSeries(
    out.filter((m): m is RuleBarsMember => !!m && wanted.has(m.symbol)).map((m) => ({ ...m, shares: shares.get(m.symbol) || [] })),
    schedule.events,
  ...
  const failed = out.filter((m): m is RuleBarsMember => !!m?.error && wanted.has(m.symbol)).map((m) => ({ ticker: m.symbol, error: m.error }));
  return {
    rule: true,
    ...extra,
    ...series,
    ...
    failed,
    source: [...new Set(out.filter((m): m is RuleBarsMember => !!m && !m.error).map((m) => m.source))].join(' + ') || null,
...
function rebasedTo(hist: { days: readonly Bar[] }, start: IsoDate, end: IsoDate, base: number) {
