// Share prices for the valuation page: daily closes as quoted on each date,
// from the same source chain as the custom-ETF charts (TradingView, then IBKR
// TWS, then Yahoo) - fetched when the page is viewed, cached half an hour, no
// live quote: "now" is the last daily close. All three sources give
// split-adjusted closes; the split events (Yahoo, cached a day) turn them back
// into the prices of the day, which is what EPS and share counts of old
// filings line up with.

import { store } from './store.js';
import { dailyBars } from './bars.js';
...
// { symbol, source, currency, days: [{ date, close (as quoted then), adjClose }], splits, dividends, last: { date, close } }
export async function priceSeries(ticker) {
  const [bars, ev] = await Promise.all([dailyBars(ticker), events(ticker)]);
  const days = bars.days.map((d) => {
    let factor = 1;
    for (const s of ev.splits) if (s.date > d.date) factor *= s.ratio; // undo every split after this day
